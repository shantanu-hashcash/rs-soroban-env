...
use crate::{
    ...
    builtin_contracts::{
        ...
        hcnet_asset_contract::test_hcnet_asset_contract::TestHcnetAssetContract,
        testutils::{
...
impl HcnetAssetContractTest {
    ...
    fn default_hcnet_asset_contract_with_admin_id(
        &self,
        new_admin: &Address,
    ) -> TestHcnetAssetContract {
        let contract = self.default_hcnet_asset_contract();
        let issuer = TestSigner::account(&self.issuer_key);
        contract.set_admin(&issuer, new_admin.clone()).unwrap();
        contract
    }

    fn default_hcnet_asset_contract(&self) -> TestHcnetAssetContract {
        ...
        TestHcnetAssetContract::new_from_asset(&self.host, asset).unwrap()
    }
...
#[test]
fn test_hcnet_asset_contract_smart_roundtrip() {
    let test = HcnetAssetContractTest::setup(function_name!());
    ...
    let contract = TestHcnetAssetContract::new_from_asset(&test.host, Asset::Native).unwrap();
    let expected_hcnet_asset_contract_address =
        ScAddress::Contract(test.host.get_asset_contract_id_hash(Asset::Native).unwrap());

    assert_eq!(
        contract.address.to_sc_address().unwrap(),
        expected_hcnet_asset_contract_address
    );
...
fn test_asset_init(testname: &'static str, asset_code: &[u8]) {
    let test = HcnetAssetContractTest::setup(testname);
    let issuer_id = signing_key_to_account_id(&test.issuer_key);
    let asset = create_asset(&issuer_id, asset_code);

    let contract = TestHcnetAssetContract::new_from_asset(&test.host, asset.clone()).unwrap();
    let expected_hcnet_asset_contract_address =
...
fn test_do_not_init_contract_for_invalid_asset() {
    ...
    let run_test = |asset_code| {
        let asset = create_asset(&issuer_id, asset_code);
        let res = TestHcnetAssetContract::new_from_asset(&test.host, asset.clone());
        assert!(res.is_err());
...
fn test_zero_amounts() {
    ...
    let contract = test.default_hcnet_asset_contract();
...
#[test]
fn test_hcnet_asset_contract_authorization() {
    let test = HcnetAssetContractTest::setup(function_name!());
    let admin = TestSigner::account(&test.issuer_key);
    let contract = test.default_hcnet_asset_contract();
...
fn test_trustline_auth() {
    ...
    let user = TestSigner::account_with_multisig(&user_acc_id, vec![&test.user_key]);
    let contract = test.default_hcnet_asset_contract_with_admin_id(&admin.address(&test.host));
...
fn test_account_invoker_auth_with_issuer_admin() {
    ...
    let contract = test.default_hcnet_asset_contract_with_admin_id(&admin_address);
    // create a trustline for user_acc so the issuer can mint into it
    test.create_trustline(
        &user_acc,
        &admin_acc,
...
fn test_contract_invoker_auth() {
    ...
    .unwrap();
    let contract = test.default_hcnet_asset_contract_with_admin_id(&admin_contract_address);
...
fn test_hcnet_asset_contract_classic_balance_boundaries(
    test: &HcnetAssetContractTest,
    user: &TestSigner,
    account_id: &AccountId,
    init_balance: i64,
...
#[test]
fn test_hcnet_asset_contract_classic_balance_boundaries_simple() {
    let test = HcnetAssetContractTest::setup(function_name!());
    ...
    test_hcnet_asset_contract_classic_balance_boundaries(
        &test,
        &user,
        &account_id,
        100_000_000,
...
#[test]
fn test_hcnet_asset_contract_classic_balance_boundaries_with_liabilities() {
    let test = HcnetAssetContractTest::setup(function_name!());
    let account_id = signing_key_to_account_id(&test.user_key);
    let user = TestSigner::account_with_multisig(&account_id, vec![&test.user_key]);
    test.create_account(
...
#[test]
fn test_hcnet_asset_contract_classic_balance_boundaries_with_sponsorships() {
    let test = HcnetAssetContractTest::setup(function_name!());
    let account_id = signing_key_to_account_id(&test.user_key);
    let user = TestSigner::account_with_multisig(&account_id, vec![&test.user_key]);
    test.create_account(
...
#[test]
fn test_hcnet_asset_contract_classic_balance_boundaries_with_sponsorships_and_liabilities() {
    let test = HcnetAssetContractTest::setup(function_name!());
    let account_id = signing_key_to_account_id(&test.user_key);
    let user = TestSigner::account_with_multisig(&account_id, vec![&test.user_key]);
    test.create_account(
...
#[test]
fn test_hcnet_asset_contract_classic_balance_boundaries_with_large_values() {
    let test = HcnetAssetContractTest::setup(function_name!());
    let account_id = signing_key_to_account_id(&test.user_key);
    let user = TestSigner::account_with_multisig(&account_id, vec![&test.user_key]);
    test.create_account(
...
fn test_wrapped_asset_classic_balance_boundaries(
    ...
    let contract = TestHcnetAssetContract::new_from_asset(
        &test.host,
        Asset::CreditAlphanum12(AlphaNum12 {
...
#[test]
fn test_asset_hcnet_asset_contract_classic_balance_boundaries_simple() {
...
#[test]
fn test_asset_hcnet_asset_contract_classic_balance_boundaries_with_trustline_limit() {
    test_wrapped_asset_classic_balance_boundaries(
        function_name!(),
        100_000_000,
        0,
...
#[test]
fn test_asset_hcnet_asset_contract_classic_balance_boundaries_with_liabilities() {
    test_wrapped_asset_classic_balance_boundaries(
        function_name!(),
        100_000_000,
        300_000,
...
#[test]
fn test_asset_hcnet_asset_contract_classic_balance_boundaries_with_limit_and_liabilities() {
    test_wrapped_asset_classic_balance_boundaries(
        function_name!(),
        100_000_000,
        300_000,
...
#[test]
fn test_asset_hcnet_asset_contract_classic_balance_boundaries_large_values() {
    test_wrapped_asset_classic_balance_boundaries(
        function_name!(),
        i64::MAX - i64::MAX / 5,
        i64::MAX / 4,
...
#[test]
fn test_recording_auth_for_hcnet_asset_contract() {
    let test = HcnetAssetContractTest::setup(function_name!());
...
fn test_sac_reentry_is_not_allowed() {
    ...
    let contract_1 = TestHcnetAssetContract::new_from_asset(&test.host, asset_1).unwrap();
    let contract_2 = TestHcnetAssetContract::new_from_asset(&test.host, asset_2).unwrap();
