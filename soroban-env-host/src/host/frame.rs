...
pub(crate) enum Frame {
    ...
    HcnetAssetContract(Hash, Symbol, Vec<Val>, ScContractInstance),
    #[cfg(any(test, feature = "testutils"))]
...
impl Frame {
    fn contract_id(&self) -> Option<&Hash> {
        match self {
            ...
            Frame::HcnetAssetContract(id, ..) => Some(id),
            #[cfg(any(test, feature = "testutils"))]
    ...
    fn instance(&self) -> Option<&ScContractInstance> {
        match self {
            ...
            Frame::HostFunction(_) => None,
            Frame::HcnetAssetContract(_, _, _, instance) => Some(instance),
            #[cfg(any(test, feature = "testutils"))]
...
impl Host {
    ...
    fn call_contract_fn(&self, id: &Hash, func: &Symbol, args: &[Val]) -> Result<Val, HostError> {
        ...
        match &instance.executable {
            ...
            ContractExecutable::HcnetAsset => self.with_frame(
                Frame::HcnetAssetContract(id.metered_clone(self)?, *func, args_vec, instance),
                || {
                    use crate::builtin_contracts::{BuiltinContract, HcnetAssetContract};
                    HcnetAssetContract.call(func, self, args)
                },
            ),
        }
    }
