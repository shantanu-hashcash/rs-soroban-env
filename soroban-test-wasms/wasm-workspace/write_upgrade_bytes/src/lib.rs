...
/*
This contract is used by a hcnet core test case to do a Soroban ConfigSetting upgrade.
A copy of this contract also exists in the scripts directory of hcnet-core, which is used
by a python script to execute the upgrade. If there's an easy way for the python script to use
this contract in env, the one in hcnet-core should be deleted.
*/
#[contractimpl]
